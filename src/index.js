...
export function getSystemInfo() {
    console.log('当前站点 ' + document.domain + ' 使用 畅码后台管理系统 框架技术')
    console.log('Cma - PrintCma')
}
var DEFAULT_OPTIONS = {
...
/** PrintCma class that prints HTML elements in a blank document */
var PrintCma = /** @class */ (function () {
    function PrintCma(options) {
    ...
    PrintCma.prototype.getIFrame = function () {
        return this.iframe;
    };
    /**
     ...
     */
    PrintCma.prototype.print = function (el, styles, scripts, callback) {
        if (this.isLoading)
            return;
        ...
        doc.open();
        doc.write('<!DOCTYPE html><html lang="zh"><head><title>Cma</title></head><body/></html>');
        this.addEvents();
     ...
     * @param callback Optional callback that will be triggered when content is ready to print
     */
    PrintCma.prototype.printURL = function (url, callback) {
        if (this.isLoading)
            return;
        this.addEvents();
        this.isLoading = true;
    ...
    PrintCma.prototype.launchPrint = function (contentWindow) {
        var result = contentWindow.document.execCommand('print', false, null);
        if (!result) {
            contentWindow.print();
        }
    };
    PrintCma.prototype.addEvents = function () {
        var _this = this;
        if (!this.hasEvents) {
            this.hasEvents = true;
    ...
    PrintCma.prototype.onLoad = function () {
        var _this = this;
        if (this.iframe) {
            this.isLoading = false;
